...
class ReminderServiceTest(test_base.FakeDatabaseTest):
    ...
    def test_create_reminder_for_existing_email(self):
        email = 'blah@blah.com'
    ...
    def test_update_reminder_with_valid_token(self):
        email = 'foo@foo.com'
        existing_db_reminder = create_db_reminder(
    ...
    def test_update_token_normalizes_email(self):
        existing_db_reminder = create_db_reminder(
        ...
        req = reminder.UpdateReminderRequest(
            email=' foo@FOO.COM',
            status=reminder.Status.UNSUBSCRIBED,
            token=reminder_service.generate_email_token('FOO@foo.com '))
        resp = self.service().invoke('update', req)
    ...
    def test_update_reminder_with_invalid_token(self):
        req = reminder.UpdateReminderRequest(
            email='foo@foo.com',
            status=reminder.Status.UNSUBSCRIBED,
            token='garbage')
    ...
    def test_update_reminder_with_expired_token(self, mock_utcnow):
        email = 'foo@foo.com'
